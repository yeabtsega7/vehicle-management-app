import 'package:bit_vehicle_web/compnent/driverCard.dart';
...
class dispatcherDriverPage extends StatefulWidget {
  const dispatcherDriverPage({Key? key}) : super(key: key);

  @override
  State<dispatcherDriverPage> createState() => _dispatcherDriverPageState();
}

class _dispatcherDriverPageState extends State<dispatcherDriverPage> {
...
class _AddUserState extends State<AddUser> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text("Licence Number",style: TextStyle(fontSize: 15,color: AppColors.lightFontColor,fontWeight: FontWeight.w700),),
          const SizedBox(height: 10,),
          Container(
            // width: width,
            height: 40,
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text(licenceNumber,style: const TextStyle(color: Colors.red,fontSize: 10),)
            ],
          ),const SizedBox(height: 10,),
  ...
  Future<void> addUserToDb() async {
    ...
    if(addUserValidation()){
      try {
        ...
        ).then((value)
        {
          ...
          firebaseCone.driverRef.doc(value.user!.uid)
              .set({
            "id":value.user!.uid,
    ...
    licenceNumberFiledControler.clear();
