import 'package:bit_vehicle_web/compnent/officeManagerCard.dart';
...
class dispatcherOfficeManagerPage extends StatefulWidget {
  const dispatcherOfficeManagerPage({Key? key}) : super(key: key);

  @override
  State<dispatcherOfficeManagerPage> createState() => _dispatcherOfficeManagerPageState();
}

class _dispatcherOfficeManagerPageState extends State<dispatcherOfficeManagerPage> {
                                    ...
                                    Container(
                                      child:ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          return officeManagerCard(doc: data[i]);
                                        },
                                      ),
                                    ),
                                    Container(
...
class _AddUserState extends State<AddUser> {
  ...
  final userOfficeFiledControler = TextEditingController();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text("Department",style: TextStyle(fontSize: 15,color: AppColors.lightFontColor,fontWeight: FontWeight.w700),),
          const SizedBox(height: 10,),
          Container(
            // width: width,
            height: 64,
          ...
          const Text("office Number",style: TextStyle(fontSize: 15,color: AppColors.lightFontColor,fontWeight: FontWeight.w700),),
          const SizedBox(height: 10,),
          Container(
            // width: width,
            height: 40,
            ...
            child: TextFormField(
              controller: userOfficeFiledControler,
              decoration: const InputDecoration(
                  border: InputBorder.none,
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text(userOffice,style: const TextStyle(color: Colors.red,fontSize: 10),)
            ],
          ),
  ...
  Future<void> addUserToDb() async {
    ...
    if(addUserValidation()){
      try {
        ...
        {
          firebaseCone.officeManagerRef.doc(value.user!.uid)
              .set({
            "id":value.user!.uid,
            ...
            "officeNumber":userOfficeFiledControler.text,
            "active":true
          })
              .then((value){
    ...
    userOfficeFiledControler.clear();
  ...
  bool addUserValidation(){
    bool valid =true;
    if(userOfficeFiledControler.text.trim()==""){
      setState(() {
        userOffice="invalid input";
      });
      valid=false;
    }else{
      setState(() {
