...
class officeRequestCard extends StatelessWidget {
  final QueryDocumentSnapshot doc;

  const officeRequestCard({Key? key, required this.doc}) : super(key: key);
              ...
              MaterialButton(
                ...
                onPressed: () async {
                    await firebaseCone.serviceRef.doc(doc.id).update({
                      "officeApproved":true,
                      "officeDis":false,
                      "officeManagerName": FirebaseAuth.instance.currentUser!.email
                    }).then((value) async {
              ...
              MaterialButton(
                ...
                onPressed: () async {
                  await firebaseCone.serviceRef.doc(doc.id).update({
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Request Disapproved"),backgroundColor: Colors.green,)
                    );
                  });
                  Navigator.pop(context);
                },
